  ...
  switch( event ) {
...
#endif
        syslog( LOG_INFO, "time=%s event=announce info_hash=%s peer_id=%s ip=%s", timestring, hash_hex, ws->peer_id, ip_readable );
      }
#endif
    ...
    case EVENT_COMPLETED:
#ifdef WANT_SYSLOGS
      {
        ...
        ip_readable[ fmt_ip4( ip_readable, (char*)&ws->peer ) ] = 0;
#endif
        syslog( LOG_INFO, "time=%s event=completed info_hash=%s peer_id=%s ip=%s", timestring, hash_hex, ws->peer_id, ip_readable );
      }
#endif
      ot_overall_completed++;
      break;
    case EVENT_SCRAPE:
#ifdef WANT_SYSLOGS
      {
        ...
        ip_readable[ fmt_ip4( ip_readable, (char*)&ws->peer ) ] = 0;
#endif
        syslog( LOG_INFO, "time=%s event=scrape ip=%s", timestring, ip_readable );
      }
#endif
